...
import dummyData from "../variables/derivativesDummy";
import {
  createColumnHelper,
  flexRender,
  getCoreRowModel,
...
import AddDerivativesAccount from "./AddDerivativesAccount";
import { toast, Toaster } from "react-hot-toast";
import AccountSize from "./AccountSize/index";
import { MdBlock } from "react-icons/md";
import axios from "axios";
import DerivativesEdit from "./DerivativesEdit"
import DerivativesDelete from "./DerivativesDelete"
...
function DerivativesTable() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.get(
        "https://capital-curve-apis.onrender.com/api/account-type/get-all-Derivative-plan-and-addons-admin",
        {
          headers: {
    ...
    columnHelper.accessor("status", {
      ...
      cell: (info) => (
        <div className="flex items-center justify-center">
          <p className={`text-sm font-bold w-16 text-gray-700 dark:text-navy-700 rounded-md text-center ${info.getValue() === "Active" ? "bg-green-200" : "bg-red-200"}`}>{info.getValue()}</p>
        </div>
      ),
    }),
  ...
  // const handleAddNewData = async (newData) => {
  //   if (!newData) {
  ...
  //       setDataTable2((prev) => [...prev, newData]);
