...
function DeleteAddons({ isOpen, Addons, onClose, onDelete }) {
    const [formData, setFormData] = useState({
        status: ""
    });

    if (!isOpen || !Addons) return null;
    ...
    const handleDelete = async () => {
        try {
            ...
            const response = await axios.post(
                `https://capital-curve-apis.onrender.com/api/addons/delete-addons/${Addons._id}`,
                updatedFormData,
                {
                    headers: {
            ...
            toast.error("Error deleting Addons ");
        }
    };
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-white dark:bg-navy-900 p-6 rounded-lg border border-blueSecondary dark:border-brand-400">
                <h2 className="text-xl font-bold text-gray-700 dark:text-white mb-4">Delete Addons </h2>
                <p className="text-gray-700 dark:text-gray-300">
                    Are you sure you want to delete this Addons ?
                </p>
